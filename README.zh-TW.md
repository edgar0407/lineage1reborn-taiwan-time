...
| 位置 | 網站顯示 | 轉換後顯示 |
| --- | --- | --- |
| 頁首「Time Now」時鐘 | `09:50:36 PM EDT` | `09:50:36 AM TW` |
| 活動倒數結束時間 | `Ends Jul 30, 10:00pm ET` | `Ends Jul 31, 10:00 TW` |
| 城堡「Next Siege」時間 | `1 Aug 10 AM ET` | `1 Aug 22:00 TW` |
...
- 目前僅針對首頁（`/`）驗證過實際網站標記（markup）。
- 若 lineage1reborn.com 更改 DOM 結構或文字內容，攻城時間的比對邏輯（依賴文字樣式而非穩定 selector）可能失效——失效時會靜默跳過，不會顯示錯誤資料。
